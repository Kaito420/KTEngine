...
void Scene::RenderHierarchy()
{
	ImGui::Begin("Hierarchy View"); 
	{
		...
		for (auto it = _gameObjects.begin(); it != _gameObjects.end(); it++, index++) {
			...
			ImGui::PushID(gameObject->_id); //ӂIDݒ
			//I
			...
			if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
				GameObject* srcPtr = gameObject.get(); // shared_ptr̒g̐|C^
			...
			if (ImGui::BeginDragDropTarget()) {
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DND_GAMEOBJECT")) {
					...
					if (srcIt != _gameObjects.end() && srcIt != it) {
						//src  target ̑Oɂ邩𒲂ׂisrcBeforeTarget  true ȂOj
						...
						// }ʒu߂iOȂ target ̎ɁAȂ target ̑OɁj
						auto insertPos = it;
						if (srcBeforeTarget) ++insertPos; // target ̌ɓ
		...
		if (ImGui::BeginPopupContextWindow(nullptr, 
			...
			if(ImGui::MenuItem("Create Empty")){
				...
				_selectedObjId = newObj->_id; //VK쐬IuWFNgI
			}
		...
		if (_selectedObjId != -1) {
			...
			if (selectedObj) {
				...
				for (auto& component : selectedObj->_components) {
					ImGui::PushID(component.get()); //IDՓ˖h~

					bool open = ImGui::TreeNode(component->GetComponentName().c_str());

					if (ImGui::BeginPopupContextItem()) {
						if (ImGui::MenuItem("Remove Component")) {
							componentToRemove = component.get(); //폜\
						}
						ImGui::EndPopup();
					}
